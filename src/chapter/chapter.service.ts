...
export class ChapterService implements ChapterServiceInterface {
    ...
    async updateStatusChapter(payload: UpdateStatusChapterDto): Promise<ChapterResponse> {
        const user = await this.prismaService.user.findUnique({
            where: {
                email: payload.email
            }
    ...
    async findChapterByToken (payload: FindChapterDto): Promise<Chapter>{
        const course = await this.findCourseBySlug(payload.course_slug, payload.email);
    ...
    async updateValueChapter(payload: UpdateValueChapterDto): Promise<ChapterResponse> {
        const user = await this.prismaService.user.findUnique({
            where: {
                email: payload.email
            }
